...
function formVerification() {    // проверка формы и запись в массив.
    // не реализована проверка диапазона введенных значений 
    ...
    if(type.value == 'rgb' && /^(\d{1,3}),(\d{1,3}),(\d{1,3})$/gi.test(`${code.value}`)) {     // тут проблема с регуляркой . не могу составить. пишу только цифры
        data.type ='rgb'
        data.code = `${code.value}`
        status2 = true
    }
    if(type.value == 'rgba' && /^(\d{1,3}),(\d{1,3}),(\d{1,3}),(\d\.?\d?)$/gi.test(`${code.value}`)) {     // тут проблема с регуляркой . не могу составить. пишу только цифры
        data.type ='rgba'
        data.code = `${code.value}`
        status2 = true
    }
    if(type.value == 'hex' && /^#([0-9a-fA-F]{3}){1,2}$/gi.test(`${code.value}`)) {     // тут проблема с регуляркой . не могу составить. пишу только цифры
        data.type ='hex'
        data.code = `${code.value}`
        status2 = true
    } else 
